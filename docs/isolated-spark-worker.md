...
2026-09-19首次真实运行`35427706320`在GitHub Linux Runner完成Python3.10哈希依赖安装、协议测试、Spark3.5.9 JAR检查、秘密文件检查和短期Artifact上传。ZIP约303MiB并设置1天到期；该产物仅用于受控云验证，不提交Git。Node24 Action升级后的运行`35428122388`同样成功且弃用告警消失。两次GitHub Artifact摘要不同，但该摘要属于`upload-artifact`外层封装，不能据此判断内部Worker ZIP是否不同；早先将其直接解释为ZIP不可复现，证据不足。构建脚本随后固定1980时间戳、字节序排序并使用`zip -X`，工作流直接记录内部ZIP摘要。独立运行`35429097554`和`35429097543`均生成318,395,161字节的内部ZIP，SHA-256同为`15c13c309c39203bee1e2d94d59d39f69a594972fffcb928c2609d5fdb4d2f4a`，W1可复现构建通过。
...
| W1 Linux包 | Python3.10构建、哈希依赖、ZIP<480MiB、Spark3.5.9 JAR | 已验收；双构建内部SHA-256一致 |
