...
- `V2_MYSQL_POOL_SIZE`、`V2_MYSQL_CONNECT_TIMEOUT_MS`、`V2_MYSQL_CONNECT_ATTEMPTS`、`V2_MYSQL_RETRY_DELAY_MS`、`V2_MYSQL_SSL`

Serverless RDS自动暂停后，云函数冷启动可能先遇到连接超时或拒绝。后端仅对受限瞬态错误执行有界重试，默认6次、单次连接5秒、2秒线性退避并限制总时间；鉴权、库权限和SQL错误不会重试。所有数值配置在建池前校验，最终错误只保留固定代码，不携带主机、账号或原始消息。
...
OSS读取和条件写入默认最多3次、500毫秒线性退避；只重试网络失败、429和5xx，每次重新生成签名。404、409、412不作为瞬态错误。若条件写已经在远端成功但响应丢失，重试得到412时只在远端修订和正文与目标完全一致后返回幂等成功；来自另一个实例的普通陈旧写仍返回冲突。不可变产物使用相同网络重试，但同摘要不同正文永不覆盖。
...
部署包、GitHub环境配置及部署后核验见[V2云部署说明](v2-cloud-deployment.md)。
