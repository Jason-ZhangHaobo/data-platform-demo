...
在当前 MVP 闭环完成后，进入第二个总体目标：由一个人借助 Codex，在 12 个月上限内，把平台演进为面向中国证券公司的综合数据平台，优先围绕经纪与财富管理、资产管理与基金，重点提升 Data Agent、产品易用性和功能完整度。详细差距矩阵、财富顾问持仓标杆旅程和路线图见[世界级数据中台差距与演进路线](second-goal-gap-and-roadmap.md)。
...
- 数据运维诊断 Skill。
- 数据质量与运维 Skill（当前先交付质量规则、得分、时效提醒和检查历史 MVP）。
