...
| 元数据与治理 | 版本化目录、表级血缘、同步字段映射、指标/标准/契约入口 | L2 | Spark SQL字段级解析、技术/业务血缘统一、影响分析、责任人/术语/认证、自动Profiling |
...
4. 实现Spark SQL字段级血缘、变更影响、数据契约兼容检查、自动Profiling与资产认证，把Agent上下文从“字段列表”升级为可信知识图谱。
