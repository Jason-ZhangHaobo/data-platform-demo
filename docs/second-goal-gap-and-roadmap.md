...
|---|---|---:|---|
| Data Agent开发闭环 | 真实Qwen生成、Spark执行/报错修复、版本Diff；新任务可后台自动生成不可变调度/部署包并实际文件演练，停在人工审阅；冻结20例本机完整链路100% | L2+ | 受控发布和上线后运维Agent、更多输出契约、跨模块恢复/反馈、技能/指令管理、云隔离执行 |
...
| Agent可信度 | 独立断言、最多3次修复、批准绑定哈希、七阶段只读证据旅程和人工责任边界、失败/阻塞/救援留存 | L2+ | 真实跨模块Agent编排Trace与重放、逐步工具输入输出摘要、用户反馈、风险分级审批、模型/Prompt A/B与成本质量看板 |
