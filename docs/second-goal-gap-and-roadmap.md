...
数舵V2当前为**L2（可信单机场景）**：不是早期假数据原型，也还不是可交付公司的企业平台。优势是证券标杆链路证据严格、GUI/API/CLI/MCP同源、统一Agent到专业Agent的受控路由/交接、失败关闭和人工审阅边界清晰；最大短板是公网真实云、隔离Spark、多源数据面规模和企业治理尚未验收。

近期目标是先到L3，不以一年为固定工期；L3门槛通过后，再按公司真实基础设施评估L4。全面“超过DataWorks”不是可验证的短期结论，应改成在选定证券任务上完成同任务对照，并争取更少人工步骤、更清楚证据和更高成功率。

## 世界级产品给出的最新设计信号

| 对标 | 官方能力信号 | 数舵应吸收的原则 | 不应误读 |
|---|---|---|---|
...
|---|---|---:|---|
| Data Agent开发闭环 | 真实Qwen生成、Spark执行/报错修复、版本Diff；新任务可后台自动生成不可变调度/部署包并实际文件演练，停在人工审阅；统一入口已真实路由“资产理解→报表设计”、交接可审计且不自动执行；冻结20例本机完整链路100% | L2+ | 受控发布和上线后运维Agent、更多输出契约、跨模块恢复/反馈、技能/指令管理、云隔离执行 |
| 经典功能广度 | 接入、离线/实时、开发、调度、资产、质量、安全、报表、DAPI/XAPI、运维均有真实合成用例；SERVER_MYSQL具备服务端白名单Schema扫描、有界FULL和全表快照差异UPSERT代码/虚构API验收 | L2 | 真实同VPC MySQL数据面、源端增量/CDC、对象存储、Kafka/Flink、Python、BI连接器及规模/兼容矩阵 |
...
| Agent可信度 | 独立断言、最多3次修复、批准绑定哈希、七阶段只读证据旅程和人工责任边界、失败/阻塞/救援留存；意图原文不持久化/不回显、秘密模式拒绝、成员任务隔离 | L2+ | 真实跨模块Agent编排Trace与重放、逐步工具输入输出摘要、用户反馈、风险分级审批、模型/Prompt A/B与成本质量看板 |
...
3. 完成SERVER_MYSQL真实同VPC数据面验收，并新增OSS、Kafka/Flink和CDC适配；每种连接器先过经典人工/API验收，再开放给Agent。
...
每个任务同时记录：首个正确结果耗时、Agent工具调用数、人工批准次数、返工次数、最终断言、权限事件、Token/计算成本和可恢复性。没有厂商同环境结果时只报告数舵自身数据，不凭宣传页推导“超过”。
