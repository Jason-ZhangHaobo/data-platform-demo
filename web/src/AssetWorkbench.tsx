  ...
  return (
    <div className="asset-workbench-v2">
      ...
      <section className="asset-agent-v2">
        <header><div className="asset-agent-icon-v2"><Bot size={20} /></div><div><span className="eyebrow">DATA AGENT · GROUNDED DISCOVERY</span><h3>找数据、解释口径、看下游影响</h3><p>模型只接收资产摘要和版本绑定关系；引用未知资产即失败，不读取业务行。</p></div></header>
      ...
      <div className="asset-main-v2">
        ...
        <section className="asset-detail-v2">
          {detail ? <>
            ...
            <div className="asset-tabs-v2" role="tablist" aria-label="资产详情视图">{[["overview", "概览"], ["fields", `字段 ${detail.fields.length}`], ["lineage", `血缘 ${detail.lineage.edges.length}`], ["governance", "指标与标准"]].map(([id, label]) => <button key={id} role="tab" aria-selected={tab === id} className={tab === id ? "active" : ""} onClick={() => setTab(id as typeof tab)}>{label}</button>)}</div>
      ...
      <footer className="asset-boundary-v2"><GitBranch size={14} />目录与血缘来自当前本机版本绑定；全部数据为虚构证券样例，未连接公司目录、真实用户或公网资产。</footer>
    </div>
  );
}
