...
const requestedView = () => {
  ...
  if (["home", "sources", "sync", "realtime", "development", "quality", "assets", "masking", "holdings-report", "ops", "security", "agent"].includes(view)) return view;
  return window.location.hash === "#development" ? "development" : "home";
};
...
const primaryNavItems = [["home", "工作台"], ["sources", "数据源"], ["sync", "数据同步"], ["realtime", "实时同步"], ["development", "数据开发"], ["quality", "数据质量"], ["assets", "数据资产"], ["masking", "数据脱敏"], ["holdings-report", "数据报表"], ["ops", "运维监控"], ["security", "账号权限"], ["agent", "Data Agent"]];
...
async function refreshAssets() {
  ...
  [state.assets, state.dataSources, state.dataContracts] = await Promise.all([request(`/api/assets${suffix}`), request("/api/sources"), request("/api/contracts")]);
...
async function refreshQuality() {
  [state.qualityRules, state.qualitySummary, state.assets] = await Promise.all([request("/api/quality/rules"), request("/api/quality/summary"), request("/api/assets")]);
...
async function refreshHome() { [state.tasks, state.summary, state.devJobs, state.qualitySummary, state.assets, state.dataSources, state.streamJobs, state.opsIncidents] = await Promise.all([request("/api/tasks"), request("/api/summary"), request("/api/dev/jobs"), request("/api/quality/summary"), request("/api/assets"), request("/api/sources"), request("/api/stream/jobs"), request("/api/ops/incidents")]); render(); }

const refreshCurrentView = () => state.view === "home" ? refreshHome() : state.view === "sources" ? refreshSources() : state.view === "development" ? refreshDevelopment() : state.view === "masking" ? refreshMasking() : state.view === "assets" ? refreshAssets() : state.view === "security" ? refreshSecurity() : state.view === "agent" ? refreshAgent() : state.view === "quality" ? refreshQuality() : state.view === "holdings-report" ? refreshHoldingsReport() : state.view === "realtime" ? refreshRealtime() : state.view === "ops" ? refreshOps() : refresh();
...
function renderMasking() {
  ...
  </main><footer><span>数舵 Data Platform Lab</span><span>虚构证券行业数据 · 学习环境 · 禁止接入真实生产</span></footer>${state.maskingEditing !== undefined ? maskingRuleForm(state.maskingEditing) : ""}</div>`;
}
...
function renderAssetDetail(asset) {
  ...
  const contract = state.dataContracts.find((item) => item.assetPhysicalName === asset.physicalName);
  ...
  const contractContext = contract ? `<div class="contract-context"><div><strong>${escapeHtml(contract.name)} · v${escapeHtml(contract.version)}</strong><span class="status-badge ${contract.status === "ACTIVE" ? "success" : "info"}">${contract.status === "ACTIVE" ? "已生效" : "草稿待评审"}</span></div><p>兼容策略：${escapeHtml(contract.compatibility)} · 负责人：${escapeHtml(contract.owner)}</p><p>质量 SLO：${escapeHtml(contract.qualitySlo)}</p><p>下游消费者：${contract.consumers.map(escapeHtml).join("、")}</p><small>${escapeHtml(contract.changePolicy)}</small></div>` : `<div class="contract-context empty"><strong>尚未登记数据契约</strong><span>请先明确字段、SLO、消费者与变更策略。</span></div>`;
  return `<div class="asset-detail"><div class="selected-task-summary"><span class="source-icon">表</span><div><strong>${escapeHtml(asset.name)}</strong><small>${escapeHtml(asset.description)}</small></div></div><div class="asset-detail-meta"><span class="status-badge ${tone}">${label}</span><span>${escapeHtml(asset.assetType)} · ${escapeHtml(asset.layer)}</span><span>负责人：${escapeHtml(asset.owner)}</span></div>${metadataContext}${contractContext}<div class="lineage-block"><p class="eyebrow">LINEAGE</p><p>${asset.upstream.length ? asset.upstream.map((item) => `<span class="lineage-chip">${escapeHtml(item)}</span>`).join(" <span class=\"lineage-arrow\">→</span> ") : "暂无上游登记"}</p></div><div class="field-table"><div class="field-table-row field-table-head"><span>字段</span><span>业务名称</span><span>类型</span><span>敏感等级</span></div>${asset.fields.map((field) => { const [fieldLabel, fieldTone] = assetSensitivityMeta(field.sensitivity); return `<div class="field-table-row"><span><code>${escapeHtml(field.name)}</code></span><span>${escapeHtml(field.label)}</span><span>${escapeHtml(field.type)}</span><span class="status-badge ${fieldTone}">${fieldLabel}</span></div>`; }).join("")}</div></div>`;
}
...
function renderAssets() {
  return `<div class="app-shell"><header class="topbar"><a class="brand" href="#top"><span class="brand-mark">舵</span><span><strong>数舵</strong><small>DATA PLATFORM LAB</small></span></a><nav><a class="nav-item" href="?view=sync#tasks">同步任务</a><a class="nav-item" href="?view=development#development">数据开发</a><a class="nav-item" href="?view=masking#masking">数据脱敏</a><a class="nav-item active" href="?view=assets#assets">数据资产</a><a class="nav-item" href="?view=security#security">账号权限</a></nav><div class="environment-pill"><span></span>本地演示环境</div></header><main id="top">
    <section class="hero"><div><p class="eyebrow">DATA ASSET CATALOG · MVP 0.2</p><h1>让每张表都有<br>可理解、可约束的上下文。</h1><p class="hero-copy">围绕证券主数据、投资者账户、订单成交、持仓和基金净值，建立字段、敏感等级、血缘和数据契约。</p></div><form class="asset-search" id="asset-search-form"><input name="q" value="${escapeHtml(state.assetQuery)}" placeholder="搜索资产、字段、业务域或标签"><button class="button button-primary" type="submit">搜索资产</button></form></section>
    <section class="summary-grid"><article><span>资产总数</span><strong>${state.assets.length}</strong><small>当前检索结果</small></article><article><span>受限资产</span><strong>${state.assets.filter((asset) => asset.sensitivity === "RESTRICTED").length}</strong><small>需要权限审计</small></article><article><span>字段总数</span><strong>${state.assets.reduce((sum, asset) => sum + asset.fields.length, 0)}</strong><small>已登记字段</small></article><article class="success-card"><span>已生效契约</span><strong>${state.dataContracts.filter((item) => item.status === "ACTIVE").length}</strong><small>字段与 SLO 边界</small></article></section>
    ${state.error ? `<div class="error-banner"><span>!</span>${escapeHtml(state.error)}<button data-dismiss>关闭</button></div>` : ""}
  ...
  </main><footer><span>数舵 Data Platform Lab</span><span>虚构证券行业数据 · 学习环境 · 禁止接入真实生产</span></footer></div>`;
}

function sourceForm() {
  return `<div class="modal-backdrop"><section class="task-form" role="dialog" aria-modal="true"><div class="form-heading"><div><p class="eyebrow">SOURCE ONBOARDING</p><h2>登记数据源</h2></div><button class="icon-button" data-source-close>×</button></div><form id="data-source-form"><label class="field field-wide"><span>数据源名称</span><input name="name" required minlength="2" maxlength="80" placeholder="例如：虚构证券交易 MySQL"></label><label class="field"><span>数据源类型</span><select name="sourceType"><option value="MYSQL">MySQL</option><option value="CSV">CSV 文件</option><option value="KAFKA">Kafka</option><option value="HIVE_SPARK">Hive/Spark</option></select></label><label class="field"><span>环境</span><input name="environment" required maxlength="30" value="staging"></label><label class="field field-wide"><span>连接地址或文件路径</span><input name="endpoint" required maxlength="180" placeholder="demo.example:3306/securities 或 src/shared/demo.csv"></label><label class="field"><span>负责人</span><input name="owner" required maxlength="40" value="数据集成组"></label><label class="field field-wide"><span>说明</span><textarea name="description" maxlength="200" rows="3" placeholder="仅登记虚构连接元数据；凭证须在专用密钥托管系统配置。"></textarea></label><label class="checkbox-field"><input name="enabled" type="checkbox" checked><span>登记后启用后续连接测试</span></label><div class="form-actions field-wide"><button class="button button-secondary" type="button" data-source-close>取消</button><button class="button button-primary" type="submit">登记并进入测试</button></div></form></section></div>`;
}
...
function renderSources() {
  ...
  const sourceCards = state.dataSources.map((source) => {
    const metadata = source.metadata;
    const status = source.status === "CONNECTED" ? "已连接" : source.status === "SIMULATED" ? "模拟连接" : "未测试";
    const tone = source.status === "CONNECTED" ? "success" : "neutral";
    const collection = metadata ? `<div class="metadata-source"><strong>已采集元数据</strong><span>${metadata.objectCount} 个对象 · ${metadata.fieldCount} 个字段 · ${metadata.sensitiveFieldCount} 个敏感字段</span><small>最近采集：${formatTime(metadata.collectedAt)} · 虚构环境</small></div>` : `<div class="metadata-source"><strong>尚未采集元数据</strong><span>先测试连接，再采集对象和字段摘要。</span></div>`;
    return `<article class="task-card"><div class="task-card-main"><div class="task-title-row"><span class="status-dot ${tone}"></span><h3>${escapeHtml(source.name)}</h3><span class="status-badge ${tone}">${status}</span></div><p>${escapeHtml(typeLabel[source.sourceType] ?? source.sourceType)} · ${escapeHtml(source.environment)} · ${escapeHtml(source.endpoint)}</p><div class="task-meta"><span>${escapeHtml(source.owner)}</span><span>${escapeHtml(source.description)}</span></div>${collection}</div><div class="task-actions"><button class="button button-quiet" data-source-test="${source.id}">测试连接</button><button class="button button-primary" data-source-metadata="${source.id}">采集元数据</button></div></article>`;
  }).join("");
  return `<div class="app-shell"><header class="topbar"><a class="brand" href="#top"><span class="brand-mark">舵</span><span><strong>数舵</strong><small>DATA PLATFORM LAB</small></span></a><nav></nav><div class="environment-pill"><span></span>数据源管理</div></header><main id="top"><section class="hero"><div><p class="eyebrow">DATA SOURCES · MVP 0.3</p><h1>先登记，再连接，<br>最后采集上下文。</h1><p class="hero-copy">用最少信息登记虚构数据源，随后完成连接测试和元数据采集，再交给数据资产、同步和质量模块。</p></div><button class="button button-primary hero-action" data-new-source><span>＋</span> 登记数据源</button></section><section class="summary-grid"><article><span>数据源</span><strong>${state.dataSources.length}</strong><small>已登记连接</small></article><article><span>已连接</span><strong>${state.dataSources.filter((source) => source.status === "CONNECTED").length}</strong><small>可采集元数据</small></article><article><span>已采集</span><strong>${collectedCount}</strong><small>对象与字段摘要</small></article><article class="success-card"><span>当前阶段</span><strong>0.3</strong><small>安全接入向导</small></article></section>${state.error ? `<div class="error-banner"><span>!</span>${escapeHtml(state.error)}<button data-dismiss>关闭</button></div>` : ""}<section class="workspace"><div class="panel task-panel"><div class="panel-heading"><div><p class="eyebrow">SOURCE REGISTRY</p><h2>数据源清单</h2></div><span>${state.dataSources.length} 项</span></div><div class="task-list">${sourceCards}</div></div><aside class="panel detail-panel"><div class="panel-heading"><div><p class="eyebrow">SOURCE SAFETY</p><h2>连接边界</h2></div><span>虚构环境</span></div><div class="empty-state">向导不接收密码、AK 或 Token。生产接入必须在凭证托管、网络白名单和权限审批完成后，再进行连接测试、元数据采集和敏感字段分类。</div></aside></section></main><footer><span>数舵 Data Platform Lab</span><span>虚构证券行业数据 · 禁止连接真实生产</span></footer>${state.sourceCreating !== undefined ? sourceForm() : ""}</div>`;
}
...
function renderSecurity() {
  ...
  </main><footer><span>数舵 Data Platform Lab</span><span>虚构证券行业数据 · 学习环境 · 禁止连接真实生产</span></footer></div>`;
}
...
function renderAgent() {
  ...
  return `<div class="app-shell"><header class="topbar"><a class="brand" href="#top"><span class="brand-mark">舵</span><span><strong>数舵</strong><small>DATA PLATFORM LAB</small></span></a><nav><a class="nav-item" href="?view=sync#tasks">同步任务</a><a class="nav-item" href="?view=development#development">数据开发</a><a class="nav-item" href="?view=masking#masking">数据脱敏</a><a class="nav-item" href="?view=assets#assets">数据资产</a><a class="nav-item" href="?view=security#security">账号权限</a><a class="nav-item active" href="?view=agent#agent">Data Agent</a></nav><div class="environment-pill"><span></span>规则编排演示</div></header><main id="top">
    <section class="hero"><div><p class="eyebrow">DATA AGENT LITE · MVP 0.2</p><h1>用一句话，<br>开始数据中台工作。</h1><p class="hero-copy">覆盖同步、开发、脱敏、资产、实时、持仓分析与运维诊断；每次写入或执行前都展示计划，必须由用户确认。</p></div><div class="agent-badge"><strong>7</strong><span>模块路由</span><strong>0</strong><span>外部模型调用</span></div></section>
  ...
  </main><footer><span>数舵 Data Platform Lab</span><span>虚构证券行业数据 · 学习环境 · 不连接真实生产</span></footer></div>`;
}

function qualityRuleForm() {
  const assetOptions = state.assets.map((asset) => `<option value="${asset.id}">${escapeHtml(asset.name)} · ${escapeHtml(asset.physicalName)}</option>`).join("");
  return `<div class="modal-backdrop"><section class="task-form" role="dialog" aria-modal="true"><div class="form-heading"><div><p class="eyebrow">DATA QUALITY</p><h2>新建质量规则</h2></div><button class="icon-button" data-quality-close>×</button></div><form id="quality-rule-form"><label class="field field-wide"><span>规则名称</span><input name="name" required minlength="2" maxlength="80" placeholder="例如：持仓市值不能为空"></label><label class="field"><span>数据资产</span><select name="assetId">${assetOptions}</select></label><label class="field"><span>规则类型</span><select name="ruleType"><option value="NOT_NULL">非空检查</option><option value="UNIQUE">唯一性检查</option><option value="ROW_COUNT">行数检查</option><option value="FRESHNESS">时效检查</option></select></label><label class="field"><span>字段名</span><input name="fieldName" placeholder="market_value；整表规则可留空"></label><label class="field"><span>阈值</span><input name="threshold" type="number" value="0" min="0" step="0.1"></label><label class="field"><span>负责人</span><input name="owner" required maxlength="40" value="数据质量组"></label><label class="field field-wide"><span>规则说明</span><textarea name="description" maxlength="200" rows="3" placeholder="描述口径、范围或 SLA。"></textarea></label><label class="checkbox-field"><input name="enabled" type="checkbox" checked><span>创建后启用规则</span></label><div class="form-actions field-wide"><button class="button button-secondary" type="button" data-quality-close>取消</button><button class="button button-primary" type="submit">创建规则</button></div></form></section></div>`;
}

function renderQuality() {
  const statusLabel = { PASS: "通过", WARN: "提醒", FAIL: "失败", NOT_RUN: "未运行" };
  const ruleCards = state.qualityRules.map((rule) => {
    const tone = rule.lastStatus === "PASS" ? "success" : rule.lastStatus === "WARN" ? "running" : "neutral";
    return `<article class="task-card ${rule.id === state.qualitySelectedRuleId ? "selected" : ""}" data-quality-select="${rule.id}"><div class="task-card-main"><div class="task-title-row"><span class="status-dot ${tone}"></span><h3>${escapeHtml(rule.name)}</h3><span class="status-badge ${tone}">${statusLabel[rule.lastStatus] ?? rule.lastStatus}</span></div><p>${escapeHtml(rule.assetId)} · ${escapeHtml(rule.ruleType)} · ${escapeHtml(rule.fieldName || "整表")}</p><div class="task-meta"><span>阈值 ${rule.threshold}</span><span>得分 ${rule.lastScore ?? "—"}</span><span>${escapeHtml(rule.owner)}</span></div></div><div class="task-actions"><button class="button button-quiet" data-quality-run="${rule.id}">运行检查</button><button class="button button-secondary" data-quality-toggle="${rule.id}">${rule.enabled ? "停用" : "启用"}</button></div></article>`;
  }).join("");
  const history = state.qualityRuns.slice(0, 8).map((run) => `<div class="audit-row"><span class="audit-result ${run.status === "PASS" ? "allow" : "deny"}">${statusLabel[run.status]}</span><div><strong>得分 ${run.score}</strong><small>${escapeHtml(run.observed)} · ${escapeHtml(run.message)}</small></div><time>${formatTime(run.createdAt)}</time></div>`).join("") || `<div class="empty-state">选择规则并运行一次检查。</div>`;
  return `<div class="app-shell"><header class="topbar"><a class="brand" href="#top"><span class="brand-mark">舵</span><span><strong>数舵</strong><small>DATA PLATFORM LAB</small></span></a><nav><a class="nav-item" href="?view=agent#agent">Data Agent</a><a class="nav-item active" href="?view=quality#quality">数据质量</a><a class="nav-item" href="?view=assets#assets">数据资产</a><a class="nav-item" href="?view=security#security">账号权限</a></nav><div class="environment-pill"><span></span>质量模拟环境</div></header><main id="top"><section class="hero"><div><p class="eyebrow">DATA QUALITY & OPS · MVP 0.2</p><h1>让每个资产<br>都有可解释的质量边界。</h1><p class="hero-copy">为虚构证券资产配置非空、唯一、行数和时效规则；创建、运行、提醒和运维告警形成可追溯闭环。</p></div><button class="button button-primary hero-action" data-new-quality><span>＋</span> 新建质量规则</button></section><section class="summary-grid"><article><span>规则总数</span><strong>${state.qualitySummary.totalRules ?? 0}</strong><small>资产质量规则</small></article><article><span>通过</span><strong>${state.qualitySummary.passRules ?? 0}</strong><small>最近检查</small></article><article><span>提醒</span><strong>${state.qualitySummary.warnRules ?? 0}</strong><small>需要关注</small></article><article class="success-card"><span>当前阶段</span><strong>0.2</strong><small>规则创建与运行</small></article></section>${state.error ? `<div class="error-banner"><span>!</span>${escapeHtml(state.error)}<button data-dismiss>关闭</button></div>` : ""}<section class="workspace"><div class="panel task-panel"><div class="panel-heading"><div><p class="eyebrow">QUALITY RULES</p><h2>资产质量规则</h2></div><span>${state.qualityRules.length} 项</span></div><div class="task-list">${ruleCards}</div></div><aside class="panel detail-panel"><div class="panel-heading"><div><p class="eyebrow">QUALITY HISTORY</p><h2>检查历史</h2></div><span>${state.qualityRuns.length} 条</span></div><div class="run-list">${history}</div></aside></section></main><footer><span>数舵 Data Platform Lab</span><span>虚构证券行业数据 · 学习环境 · 禁止连接真实生产</span></footer>${state.qualityCreating !== undefined ? qualityRuleForm() : ""}</div>`;
}
...
function renderHoldingsReport() {
  ...
  return `<div class="app-shell"><header class="topbar"><a class="brand" href="#top"><span class="brand-mark">舵</span><span><strong>数舵</strong><small>DATA PLATFORM LAB</small></span></a><nav><a class="nav-item" href="?view=agent#agent">Data Agent</a><a class="nav-item active" href="?view=holdings-report#holdings-report">持仓看板</a><a class="nav-item" href="?view=quality#quality">数据质量</a><a class="nav-item" href="?view=assets#assets">数据资产</a></nav><div class="environment-pill"><span></span>T+1 模拟看板</div></header><main id="top"><section class="hero"><div><p class="eyebrow">WEALTH MANAGEMENT · HOLDINGS REPORT</p><h1>财富顾问客户持仓分析</h1><p class="hero-copy">权限范围：${escapeHtml(report?.scope ?? "OWN_CLIENTS_ONLY")} · 数据时效：${escapeHtml(report?.freshness ?? "T+1 模拟数据")}</p></div><div class="report-disclaimer">${escapeHtml(report?.disclaimer ?? "虚构数据，仅用于学习演示")}</div></section>${state.error ? `<div class="error-banner"><span>!</span>${escapeHtml(state.error)}<button data-dismiss>关闭</button></div>` : ""}${reportContent}</main><footer><span>数舵 Data Platform Lab</span><span>虚构证券行业数据 · 不构成投资建议</span></footer></div>`;
}
...
function renderDevelopment() {
  ...
  </main><footer><span>数舵 Data Platform Lab</span><span>虚构数据 · 学习环境 · 禁止连接真实生产</span></footer>${state.devEditing !== undefined ? devJobForm(state.devEditing) : ""}</div>`;
}

function renderHome() {
  const modules = [
    ["数据源", "sources", state.dataSources.length, "登记、测试和采集元数据"],
    ["数据同步", "sync", state.tasks.length, "离线任务与运行记录"],
    ["实时同步", "realtime", state.streamJobs.length, "Kafka/Flink 任务与延迟"],
    ["数据开发", "development", state.devJobs.length, "SQL、依赖、发布和运行"],
    ["数据质量", "quality", state.qualitySummary.totalRules ?? 0, "规则、评分和 T+1 时效"],
    ["数据资产", "assets", state.assets.length, "元数据、契约和血缘"],
    ["数据报表", "holdings-report", 1, "持仓看板与受限问数"],
    ["运维监控", "ops", state.opsIncidents.filter((item) => item.status !== "RESOLVED").length, "告警、影响和处置手册"],
    ["Data Agent", "agent", 7, "计划、确认、代码和回归评测"],
  ];
  return `<div class="app-shell"><header class="topbar"><a class="brand" href="#top"><span class="brand-mark">舵</span><span><strong>数舵</strong><small>DATA PLATFORM LAB</small></span></a><nav></nav><div class="environment-pill"><span></span>杭州 staging · 虚构数据</div></header><main id="top"><section class="hero"><div><p class="eyebrow">SECURITIES DATA PLATFORM · WORKBENCH</p><h1>从数据接入到<br>可信业务洞察。</h1><p class="hero-copy">面向经纪与财富管理、资产管理与基金的虚构证券数据平台。每项能力都可从工作台进入，并保留权限、审计和人工确认边界。</p></div><div class="security-summary"><strong>${state.summary.successRate ?? 100}%</strong><span>同步成功率</span><strong>${state.opsIncidents.filter((item) => item.status === "OPEN").length}</strong><span>待处置告警</span></div></section><section class="summary-grid"><article><span>已连接数据源</span><strong>${state.dataSources.filter((item) => item.status === "CONNECTED").length}</strong><small>可进行元数据采集</small></article><article><span>已发布开发任务</span><strong>${state.devJobs.filter((item) => item.release?.status === "PUBLISHED").length}</strong><small>模拟调度环境</small></article><article><span>质量提醒</span><strong>${state.qualitySummary.warnRules ?? 0}</strong><small>进入运维闭环</small></article><article class="success-card"><span>Agent 回归用例</span><strong>6</strong><small>覆盖核心证券场景</small></article></section><section class="panel audit-panel"><div class="panel-heading"><div><p class="eyebrow">PLATFORM CAPABILITIES</p><h2>全链路工作入口</h2></div><span>${modules.length} 个模块</span></div><div class="workbench-grid">${modules.map(([name, view, count, description]) => `<a class="workbench-card" href="?view=${view}#${view}"><strong>${escapeHtml(name)}</strong><b>${count}</b><span>${escapeHtml(description)}</span></a>`).join("")}</div></section></main><footer><span>数舵 Data Platform Lab</span><span>虚构证券行业数据 · 不连接真实生产</span></footer></div>`;
}

function render() {
  if (state.view === "home") { app.innerHTML = renderHome(); applyPrimaryNav(); return; }
  ...
  app.innerHTML = `<div class="app-shell">
    <header class="topbar"><a class="brand" href="#top"><span class="brand-mark">舵</span><span><strong>数舵</strong><small>DATA PLATFORM LAB</small></span></a><nav><a class="nav-item active" href="?view=sync#tasks">同步任务</a><a class="nav-item" href="?view=development#development">数据开发</a><a class="nav-item" href="?view=masking#masking">数据脱敏</a><a class="nav-item" href="?view=assets#assets">数据资产</a><a class="nav-item" href="?view=security#security">账号权限</a></nav><div class="environment-pill"><span></span>本地演示环境</div></header>
    <main id="top">
    ...
    <footer><span>数舵 Data Platform Lab</span><span>虚构数据 · 学习环境 · 禁止接入真实业务</span></footer>
...
document.addEventListener("submit", async (event) => {
  ...
  if (event.target.id === "quality-rule-form") {
    event.preventDefault();
    const data = Object.fromEntries(new FormData(event.target)); data.enabled = event.target.elements.enabled.checked;
    try { const saved = await request("/api/quality/rules", { method: "POST", body: JSON.stringify(data) }); state.qualitySelectedRuleId = saved.id; state.qualityCreating = undefined; await refreshQuality(); }
    catch (error) { state.error = error.message; state.qualityCreating = undefined; render(); }
    return;
  }
  if (event.target.id === "data-source-form") {
    event.preventDefault();
    const data = Object.fromEntries(new FormData(event.target)); data.enabled = event.target.elements.enabled.checked;
    try { await request("/api/sources", { method: "POST", body: JSON.stringify(data) }); state.sourceCreating = undefined; await refreshSources(); }
    catch (error) { state.error = error.message; state.sourceCreating = undefined; render(); }
    return;
  }
