...
export async function generateIngestionPlan(
  ...
  const prompt = JSON.stringify({
    ...
    availableSources: sources.map((source) => ({
      ...
      supportsOfflineSync: source.supportsOfflineSync === true,
    ...
    contract: {
      ...
      target:
        "只能选择supportsOfflineSync=true的源；targetTable使用小写字母/数字/下划线；mapping为源字段到目标字段；keyFields引用目标字段；watermarkField可选。SERVER_MYSQL的INCREMENTAL_UPSERT首版是有界全表快照差异合并，不得描述为CDC或源端增量抽取。",
      output:
