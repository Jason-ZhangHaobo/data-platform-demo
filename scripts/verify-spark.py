...
try:
 ...
 for name,test_query,should_pass in [
  ...
  try:
   ...
   passed=actual["mainSqlExecuted"] and actual["testSqlValidation"]["passed"]==should_pass and (actual["status"]=="SUCCEEDED")==should_pass
   detail=None if passed else str(actual["validation"]["issues"])
  except Exception as error:passed=False;detail=str(error)[:500]
  results.append({"name":name,"passed":passed,"expected":"TEST_SQL_PASS" if should_pass else "TEST_SQL_FAILURE","detail":detail})
  print(("PASS " if passed else "FAIL ")+name,flush=True)
